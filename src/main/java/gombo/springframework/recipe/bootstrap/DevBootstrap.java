...
import gombo.springframework.recipe.models.*;
...
public class DevBootstrap implements ApplicationListener<ContextRefreshedEvent>
{
    ...
    private void isValidCategory (Optional<Category> cat)
    {
        if (!cat.isPresent())
        {
    ...
    private void initData()
    {
        ...
        // Get optionals
        UnitOfMeasure tablespoon = uomTbsp.get();
        UnitOfMeasure teaspoon = uomTsp.get();
        UnitOfMeasure piece = uomPiece.get();
        UnitOfMeasure dash = uomDash.get();
        UnitOfMeasure pint = uomPint.get();
        UnitOfMeasure cup = uomCup.get();

        // Get categories
        Optional<Category> catMex = categoryRepository.findByDescription("Mexican");
        Optional<Category> catAme = categoryRepository.findByDescription("American");

        // Validate categories
        isValidCategory(catMex);
        isValidCategory(catAme);

        // Get optionals
        Category mexican = catMex.get();
        Category american = catAme.get();
        ...
        guacamole.setDirections("1 Cut avocado, remove flesh: Cut the avocados in half. Remove seed. Score the inside of the avocado with a blunt knife and scoop out the flesh with a spoon. (See How to Cut and Peel an Avocado.) Place in a bowl.\n" +
                ...
                "\n" +
                "\n" +
                "Read more: http://www.simplyrecipes.com/recipes/perfect_guacamole/#ixzz4riyVzHC5");
        ...
        guacamole.getIngredients().add(new Ingredient("serrano chiles, stems and seeds removed, minced", new BigDecimal(2), piece, guacamole));
        ...
        guacamole.getIngredients().add(new Ingredient("ripe tomato, seeds and pulp removed, chopped", new BigDecimal(".5"), piece, guacamole));
        ...
        tacos.getIngredients().add(new Ingredient("Clove of Garlic, Choppedr", new BigDecimal(1), piece, tacos));
        ...
        tacos.getIngredients().add(new Ingredient("small corn tortillasr", new BigDecimal(8), piece, tacos));
        tacos.getIngredients().add(new Ingredient("packed baby arugula", new BigDecimal(3), cup, tacos));
        tacos.getIngredients().add(new Ingredient("medium ripe avocados, slic", new BigDecimal(2), piece, tacos));
        tacos.getIngredients().add(new Ingredient("radishes, thinly sliced", new BigDecimal(4), piece, tacos));
        tacos.getIngredients().add(new Ingredient("cherry tomatoes, halved", new BigDecimal(".5"), pint, tacos));
        tacos.getIngredients().add(new Ingredient("red onion, thinly sliced", new BigDecimal(".25"), piece, tacos));
        tacos.getIngredients().add(new Ingredient("Roughly chopped cilantro", new BigDecimal(4), piece, tacos));
